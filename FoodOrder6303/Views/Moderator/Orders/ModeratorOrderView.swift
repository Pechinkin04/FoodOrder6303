...
struct ModeratorOrderView: View {
    @Environment(\.presentationMode) var presentationMode
    @EnvironmentObject var modVM: ModeratorVM
...
//                .embedInTextField(title: "Адрес")
                ...
                if modVM.couriers.isEmpty {
                    Text("Нет доступных курьеров")
                        .font(.system(size: 17, weight: .medium))
                        .foregroundStyle(.textMain)
                        .embedInTextField(title: "Курьер")
                } else {
                    Menu {
                        ForEach(modVM.couriers) { courier in
                            Button {
                        ...
                        let courier = modVM.couriers.first(where: { $0.id == order.courierID })
...
#Preview {
    ModeratorOrderView(order: mockOrder)
